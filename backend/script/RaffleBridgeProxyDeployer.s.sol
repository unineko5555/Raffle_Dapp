...
/**
 * @title RaffleBridgeProxyDeployer
 * @notice RaffleBridgeのUUPSプロキシと実装コントラクトをデプロイするスクリプト
 * @dev Sepolia、Arbitrum Sepolia、Base Sepoliaにデプロイするために使用
 */
contract RaffleBridgeProxyDeployer is Script {
    // デプロイ対象のチェーンID
    ...
    function run() external {
        ...
        // 1. RaffleBridgeImplementationのデプロイ
        RaffleBridgeImplementation implementation = new RaffleBridgeImplementation();
        console.log("RaffleBridgeImplementation deployed at:", address(implementation));
        
        // 2. 初期化データの準備
        bytes memory initData = abi.encodeWithSelector(
            RaffleBridgeImplementation.initialize.selector,
            defaultRouter,
            routerAddresses,
