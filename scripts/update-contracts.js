...
const ABI_PATH = path.resolve(
  BACKEND_PATH,
  "out/RaffleImplementation.sol/RaffleImplementation.json"
);
const CONFIG_PATH = path.resolve(
  PROJECT_ROOT,
  "frontend/app/lib/contract-config.ts"
);

console.log(
  `スクリプト実行開始: ${
    isUpgradeMode ? "ABIのみを更新します..." : "デプロイ情報を更新します..."
  }`
);
      ...
      if (fs.existsSync(deployPath)) {
        ...
        if (runFiles.length > 0) {
          ...
          if (proxyTx && proxyTx.contractAddress) {
            ...
            console.log(
              `${networkName}のアドレスを取得しました: ${proxyTx.contractAddress}`
            );
          ...
          console.warn(
            `${networkName}のデプロイログファイルが見つかりませんでした`
          );
        }
      } else {
        console.warn(
          `${networkName}のデプロイディレクトリが見つかりませんでした: ${deployPath}`
        );
      }
    } catch (error) {
      console.error(
        `${networkName}のアドレス取得中にエラーが発生しました:`,
        error
      );
    }
  });
...
const loadExistingConfig = () => {
  try {
    ...
    if (configContent.includes("export const contractConfig = {")) {
      ...
      const sepoliaMatch = configContent.match(
        /11155111:\s*{[^}]+raffleProxy:\s*"([0-9a-fA-Fx]+)"/s
      );
      if (sepoliaMatch) {
      ...
      const baseMatch = configContent.match(
        /84532:\s*{[^}]+raffleProxy:\s*"([0-9a-fA-Fx]+)"/s
      );
      if (baseMatch) {
      ...
      const arbMatch = configContent.match(
        /421614:\s*{[^}]+raffleProxy:\s*"([0-9a-fA-Fx]+)"/s
      );
      if (arbMatch) {
  ...
  console.log(
    "既存の設定ファイルが見つからないか、解析できませんでした。新規作成します。"
  );
  return {};
};
...
const updateContractConfig = (newAddresses, abi, isUpgrade = false) => {
  try {
    ...
    // ERC20ABI部分を抽出して保持する
    let erc20Abi = "";
    const erc20AbiMatch = existingContent.match(
      /export\s+const\s+ERC20ABI\s*=\s*(\[\s*[\s\S]*?\n\]\s*);/
    );
    if (erc20AbiMatch) {
    ...
    // 新形式の場合は単純なマージ
    const mergedAddresses = isUpgrade
      ? existingAddresses
...
export const contractConfig = {
  ...
  11155111: {
    ...
    raffleProxy: "${
      isUpgrade
        ? existingAddresses.sepolia
        : newAddresses.sepolia ||
          existingAddresses.sepolia ||
          "0x659F54928a0Ac9EA822C356D05Ec53925A0228E8"
    }", // Sepoliaにデプロイしたプロキシアドレス
  ...
  84532: {
    ...
    raffleProxy: "${
      isUpgrade
        ? existingAddresses.baseSepolia
        : newAddresses.baseSepolia ||
          existingAddresses.baseSepolia ||
          "0xEEd88f19b0951a7BeE1B52F83Afd333eCdBB6e96"
    }", // Base Sepoliaにデプロイしたプロキシアドレス
  ...
  421614: {
    ...
    raffleProxy: "${
      isUpgrade
        ? existingAddresses.arbitrumSepolia
        : newAddresses.arbitrumSepolia ||
          existingAddresses.arbitrumSepolia ||
          "0x0573F6fE1cf8F169181eEc83Ae65BEa5502b3162"
    }", // Arbitrum Sepoliaにデプロイしたプロキシアドレス
...
export const contractAddresses = {
  ${Object.entries(mergedAddresses)
...
"stateMutability": "view"
  ...
  },
  {
    ...
    const oldAddressFormat = existingContent.includes(
      "export const contractConfig = {"
    );
    ...
    if (oldAddressFormat) {
      ...
      if (isUpgrade) {
        console.log(
          "アップグレードモード: ABIのみを更新しました。アドレスは維持されています。"
        );
...
const main = () => {
  ...
  if (!isUpgradeMode && Object.keys(addresses).length === 0) {
    console.warn(
      "新しくデプロイされたアドレスが見つかりませんでした。既存の設定を保持します。"
    );
  }
