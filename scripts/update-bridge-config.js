...
const BRIDGE_HOOK_PATH = path.join(
  __dirname,
  "../frontend/app/lib/bridge-contract-config.ts"
);
const BRIDGE_TOKEN_HOOK_PATH = path.join(
  __dirname,
  "../frontend/hooks/use-token-bridge.ts"
);
...
function loadABI(contractName) {
  ...
  try {
    ...
    console.error(
      `${COLORS.fg.red}Error loading ABI for ${contractName}:${COLORS.reset}`,
      error
    );
    process.exit(1);
  }
...
function getBridgeAddresses() {
  ...
  if (Object.keys(upgradeResults).length > 0) {
    console.log(
      `${COLORS.fg.blue}Using upgrade results from RaffleBridgeUpgrader...${COLORS.reset}`
    );
  ...
  for (const runDir of runDirs) {
    ...
    for (const networkDir of networkDirs) {
      ...
      if (
        !fs
          .statSync(path.join(BROADCAST_PATH, runDir, networkDir))
          .isDirectory()
      ) {
        continue;
      }
      ...
      if (bridgeTx) {
        ...
        console.log(
          `${COLORS.fg.green}Found Bridge Proxy on ${NETWORK_MAPPING[networkId].name}: ${COLORS.fg.cyan}${bridgeTx.contractAddress}${COLORS.reset}`
        );

        if (implementationTx) {
          bridgeAddresses[networkId].implementationAddress =
            implementationTx.contractAddress;
          console.log(
            `${COLORS.fg.green}Found Bridge Implementation on ${NETWORK_MAPPING[networkId].name}: ${COLORS.fg.cyan}${implementationTx.contractAddress}${COLORS.reset}`
          );
        }
      }
    }
...
function findUpgradeResults() {
  ...
  const upgraderRunDirs = fs
    .readdirSync(BROADCAST_PATH)
    .filter(
      (dir) => dir.includes("RaffleBridgeUpgrader") || dir.includes("Upgrader")
    );
      ...
      if (fs.existsSync(latestRunFile)) {
        try {
          ...
          if (upgradeInfo) {
            ...
            console.log(
              `${COLORS.fg.green}Found Upgrade Result for ${NETWORK_MAPPING[networkId].name}:${COLORS.reset}`
            );
            console.log(
              `${COLORS.fg.cyan}  Proxy: ${upgradeInfo.proxyAddress}${COLORS.reset}`
            );
            console.log(
              `${COLORS.fg.cyan}  Implementation: ${upgradeInfo.implementationAddress}${COLORS.reset}`
            );
          }
        } catch (error) {
          console.warn(
            `${COLORS.fg.yellow}Warning: Could not parse upgrade file for network ${networkId}${COLORS.reset}`
          );
        }
      }
    }
...
function extractUpgradeInfo(runData, networkId) {
  ...
  for (const tx of transactions) {
    ...
    if (
      tx.contractName === "RaffleBridgeImplementation" &&
...
function findBridgeTransaction(runDir, networkId) {
  ...
  for (const runFile of runFiles) {
    ...
    try {
      ...
      console.warn(
        `${COLORS.fg.yellow}Warning: Could not parse run file ${runFile}${COLORS.reset}`
      );
    }
  }
...
function updateBridgeHook(bridgeAbi, bridgeAddresses) {
  ...
  if (networks.length === 0) {
    console.error(
      `${COLORS.fg.red}Error: No bridge contracts found in broadcast files${COLORS.reset}`
    );
    process.exit(1);
  }
...
export const BRIDGE_CONFIGS: BridgeContractConfig[] = ${JSON.stringify(
    networks,
    null,
    2
  )};
...
`;
  ...
  try {
    ...
    console.log(
      `${COLORS.fg.green}Successfully updated bridge hook at:${COLORS.reset} ${BRIDGE_HOOK_PATH}`
    );
  } catch (error) {
    console.error(
      `${COLORS.fg.red}Error updating bridge hook:${COLORS.reset}`,
      error
    );
    process.exit(1);
  }
...
function updateTokenBridgeHook(bridgeAbi, bridgeAddresses) {
  try {
    ...
    if (!fs.existsSync(BRIDGE_TOKEN_HOOK_PATH)) {
      console.warn(
        `${COLORS.fg.yellow}Warning: Token bridge hook file not found at ${BRIDGE_TOKEN_HOOK_PATH}${COLORS.reset}`
      );
      return;
    }
    ...
    console.log(
      `${COLORS.fg.green}Token bridge hook is configured to import from bridge-contract-config.ts. No changes needed.${COLORS.reset}`
    );
  } catch (error) {
    console.error(
      `${COLORS.fg.red}Error updating token bridge hook:${COLORS.reset}`,
      error
    );
  }
}
...
async function main() {
  console.log(
    `${COLORS.fg.blue}Updating bridge contract configurations...${COLORS.reset}`
  );
  ...
  try {
    ...
    console.log(
      `${COLORS.fg.green}Loaded RaffleBridgeImplementation ABI${COLORS.reset}`
    );
  } catch (error) {
    console.log(
      `${COLORS.fg.yellow}RaffleBridgeImplementation ABI not found, falling back to RaffleBridge ABI${COLORS.reset}`
    );
  ...
  console.log(
    `${COLORS.fg.blue}Retrieving Bridge contract addresses...${COLORS.reset}`
  );
  ...
  console.log(
    `${COLORS.fg.green}Bridge contract configurations updated successfully!${COLORS.reset}`
  );
}
